//! DirectWrite: the Windows engine.
...
/// COM reference.
#[derive(Clone)]
pub struct Layout(IDWriteTextLayout);

impl Layout {
    /// The DirectWrite object, for the Direct2D call that draws it.
...
impl TextEngine {
    ...
    pub fn volatile(&self, text: &str) -> crate::Result<Layout> {
        Ok(Layout(self.layout_uncached(text)?))
    }
